...
describe("tool bus runtime MVP tools", () => {
  ...
  it("runs a safe shell command", async () => {
    const ctx = await makeContext();
    const result = await createDefaultToolBus().execute(
      createToolRequest({
        toolName: "shell.run",
    ...
    expect(result.status).toBe("success");
    expect(result.commandsRun).toEqual(["node --version"]);
  });
  ...
  it("resumes an interactive tool call after approval is granted", async () => {
    ...
    expect(request?.toolName).toBe("shell.run");
    if (!request) {
  ...
  it("redacts secrets from shell output events and results", async () => {
    const ctx = await makeContext();
    const seen: string[] = [];
    ctx.eventBus.subscribe((event) => {
